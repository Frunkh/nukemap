...
function plot_fallout(gz, points,color,legend) {
	...
	for(var i=0;i<points.length;i++) {
		...
		switch(MODE) {
			case GMAP: coords.push(new google.maps.LatLng(lat,lng)); break;
			case LLET: coords.push(new L.LatLng(lat,lng)); break;
		}
	...
	switch(MODE){
		...
		case LLET:
			...
			fallout_contours[det_index].push(new L.polygon(coords,{
				stroke: false,
				color: color,
				opacity: 1,
				weight: 0,
...
function do_fallout(kt, wind, fission_fraction, angle, fallout_info_div_id, airburst,hob_ft) {
		...
		if(!windsock_marker) { //create the wind marker
			...
			switch(MODE) {
				case GMAP: 
					var m_lat = marker.getPosition().lat(), m_lng = marker.getPosition().lng(), w_lat = windsock_marker.getPosition().lat(), w_lng = windsock_marker.getPosition().lng();
				break;
				case LLET:
					var m_lat = marker.getLatLng().lat, m_lng = marker.getLatLng().lng, w_lat = windsock_marker.getLatLng().lat, w_lng = windsock_marker.getLatLng().lng;					
				break;
			}
            ...
            switch(MODE) {
            	case GMAP: windsock_marker.setPosition(new google.maps.LatLng(wpos[0],wpos[1])); break;
            	case LLET: windsock_marker.setLatLng([wpos[0],wpos[1]]); break;
            }
        }
		switch(MODE) {
			case GMAP:
...
function draw_fallout() {
	...
	if(fallout_current) {
		...
		if(fallout_current.airburst&&fallout_current.kt_frac==0) {
			...
			switch(MODE) {
				case GMAP: var pos = marker.getPosition(); break;
				case LLET: var pos = marker.getLatLng(); break;
			}
			...
			if(fallout_info) {		
				...
				if(windsock_marker) {
					o+= "<small id='fallout_windsock'>Fallout windsock is "+distance(distance_between(m_lat,m_lng,w_lat,w_lng), false)+" from ground zero. <a href='#' onclick='hide_windsock(); return false;' id='hide_windsock'>Click here to hide the windsock.</a></small><br>";
				} else {
...
function new_windsock(angle) {
	...
	switch(MODE) {
		case GMAP:
			...
			var m_lat = marker.getPosition().lat(), m_lng = marker.getPosition().lng();
		break;
		case LLET:
			if(windsock_marker) {
				windsock_marker.off("dragend");
			...
			var m_lat = marker.getLatLng().lat, m_lng = marker.getLatLng().lng;
		break;
	}
    ...
    var wpos = destination_from_bearing(m_lat,m_lng,angle+180,windsock_distance);
	...
	switch(MODE) {
		...
		case LLET: 
				...
				windsock_marker.on('dragend', function() {
					var new_angle = fallout_bearing(marker.getLatLng(),windsock_marker.getLatLng());
					if(new_angle<0) new_angle+=360;
					 $("#fallout_angle").val(new_angle);
					fallout_current.angle = new_angle
						draw_fallout();
	...
	if(old_marker_pos&&windsock_marker) {
		switch(MODE) {
			...
			case LLET:
				...
				if(windsock_marker) windsock_marker.setLatLng([windsock_marker.getLatLng().lat+dlat,windsock_marker.getLatLng().lng+dlng]); 			
			break;
		}
	}
}
...
function fallout_bearing(pos1,pos2) {
	switch(MODE) {
		case GMAP: var p1lat = pos1.lat(), p1lng = pos1.lng(), p2lat = pos2.lat(), p2lng = pos2.lng(); break;
		case LLET: var p1lat = pos1.lat, p1lng = pos1.lng, p2lat = pos2.lat, p2lng = pos2.lng; break;
	}
	/*
...
/*
playback timings (ms):
  captures_list: 74.532
  exclusion.robots: 0.082
  exclusion.robots.policy: 0.074
  RedisCDXSource: 0.536
  esindex: 0.007
  LoadShardBlock: 47.765 (3)
  PetaboxLoader3.datanode: 143.808 (5)
  CDXLines.iter: 16.931 (3)
  load_resource: 160.68 (2)
  PetaboxLoader3.resolve: 34.801
*/